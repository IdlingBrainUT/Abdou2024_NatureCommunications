%% MPPCA-ICA code
function [res_PCAICA] = fxn_MPPCA_ICAv10(data_bin_z, data_bin_z_full, result_data_cell, prms_MPPCA)
%% update info
% 211104, changed xticks angle.
% 220629, Added parameter decomposing section
% 221721, customization for speed-up cal and decreased RAM usage
%% parameter decomposing section
FPS_Ca                   = prms_MPPCA.prms_ca_fps;       % RGECO: 10FPS, G-CaMP:20hz. Dependent on your data fps.
...
r_kt      = zeros(size(data_bin_z,1),r); % initialize, speed-up
r_kt_full = zeros(size(data_bin_z,1),r); % initialize, speed-up
...
for i = 1:1 % for debug
    disp('Reduce and speed-up calculation. Off to show all figures.')
% for i = 1:r

figure('Position',[100,250,700,700]); %[left bottom width height] 
...
figure('Position',[800,250,700,700]); %[left bottom width height] 
...
res_PCAICA.neuron_sig_IDs       = neuron_sig_IDs;
res_PCAICA.result_data_cell     = result_data_cell;

res_PCAICA.V_sqr_scale_z                  = V_sqr_scale_z;
res_PCAICA.r                              = r;
res_PCAICA.prms_reactivation_SD_thr       = prms_reactivation_SD_thr;
res_PCAICA.result_data_cell               = result_data_cell;
res_PCAICA.data_bin_z_full                = data_bin_z_full;
res_PCAICA.data_bin_z                     = data_bin_z;
res_PCAICA.negative2                      = negative2;
res_PCAICA.bin_frame_num                  = bin_frame_num;
res_PCAICA.FPS_Ca                         = FPS_Ca;
%%
close all
%%
end